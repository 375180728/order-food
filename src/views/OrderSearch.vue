          ...
          <List item-layout="vertical">
            <ListItem v-for="item in orderList" :key="item.sid">
              ...
              <div>
                ...
                <!-- <Rate show-text v-model="item.rate" /> -->
                <Button type="primary" @click="openRateDrawer(item)">评价</Button>
              </div>
              <template slot="action">
                <li><span>总数：</span>{{ item.oNum }}</li>
                <li><span>总价：</span>{{ item.oPrice }}</li>
...
export default {
  ...
  methods: {
    ...
    accessArival(item) {
      ...
      this.$api.user.access_arival(params).then((res) => {
        this.getOrderList();
    ...
    submitRate() {
      this.$api.user.add_rate(this.rateForm)
      .then(res => {
        this.getOrderList()
